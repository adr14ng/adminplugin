...
jQuery(document).ready( function($) {
	...
	// permalink
	if ( $('#edit-slug-box-'+post_id).length ) {
		editPermalink = function(post_id) {
			var i, c = 0, e = $('#editable-post-name-'+post_id), revert_e = e.html(), real_slug = $('#post_name-'+post_id), revert_slug = real_slug.val(), b = $('#edit-slug-buttons-'+post_id), revert_b = b.html(), full = $('#editable-post-name-full-'+post_id).html();

			$('#view-post-btn-'+post_id).hide();
			b.html('<a href="#" class="save button button-small">'+postL10n.ok+'</a> <a class="cancel" href="#">'+postL10n.cancel+'</a>');
			b.children('.save').click(function() {
				var new_slug = e.children('input').val();
				if ( new_slug == $('#editable-post-name-full-'+post_id).text() ) {
					return $('.cancel', '#edit-slug-buttons-'+post_id).click();
				}
				$.post(ajaxurl, {
					action: 'sample-permalink',
					post_id: post_id,
					new_slug: new_slug,
					new_title: $('#title-'+post_id).val(),
					samplepermalinknonce: $('#samplepermalinknonce-'+post_id).val()
				}, function(data) {
					var box = $('#edit-slug-box-'+post_id);
					box.html(data);
					...
					$('#view-post-btn-'+post_id).show();
				});
				return false;
			});

			$('.cancel', '#edit-slug-buttons-'+post_id).click(function() {
				$('#view-post-btn-'+post_id).show();
				e.html(revert_e);
				b.html(revert_b);
			...
			e.html('<input type="text" id="new-post-slug-'+post_id+'" value="'+slug_value+'" />').children('input').keypress(function(e) {
				var key = e.keyCode || 0;
		...
		makeSlugeditClickable = function() {
			$('#editable-post-name-'+post_id).click(function() {
				$('#edit-slug-buttons-'+post_id).children('.edit-slug').click();
			});
		}
		makeSlugeditClickable();
	}
